    ...
    size_t offsetR)
{
    ...
    const size_t oldCount = setO.count();
    const size_t outLen = orderL + orderR;
    setO.recount(oldCount + outLen);
    T* out = setO.ptr(oldCount);
    ...
    : order(src.order), length(0)
{
    ...
    for (size_t i = 0; i < src.length; i++)
    {
        ...
        if (isSub)
        {
            const size_t old = monomials.count();
            monomials.recount(old + src.order);
            T* dst = monomials.ptr(old);
            for (size_t k = 0; k < src.order; k++) dst[k] = m[k];
            length++;
        }
    }
}
...
void MononialSet<T>::append(TemplateSelf::vector<T>& s, size_t offset)
{
    ...
    const size_t old = monomials.count();
    monomials.recount(old + order);
    const T* src = s.ptr(offset * order);
    T* dst = monomials.ptr(old);
    for (size_t i = 0; i < order; i++) dst[i] = src[i];
    length++;
}
...
void MononialSet<T>::appendRaw(const T* mono)
{
    ...
    monomials.recount(old + order);
    T* dst = monomials.ptr(old);
    for (size_t i = 0; i < order; i++) dst[i] = mono[i];
    length++;
}
...
size_t HomogeneousPolynomial<T>::appendRaw(const T* mono, size_t coeffDelta)
{
    ...
    for (;;)
    {
        ...
        if (cell == 0)
        {
            const size_t oldCount = Set.count();
            Set.recount(oldCount + order);
            T* dst = Set.ptr(oldCount);
            for (size_t i = 0; i < order; i++) dst[i] = mono[i];
...
void HomogeneousPolynomial<T>::operator*=(const MononialSet<T>& s)
{
    ...
    for (size_t i = 0; i < length; i++)
    {
        ...
        {
            const size_t oldCount = product.count();
            MononialProduct(product, Set, order, i, s.monomials, s.order, j);
            result.appendRaw(product.ptr(oldCount), coeff);
        }
    }
